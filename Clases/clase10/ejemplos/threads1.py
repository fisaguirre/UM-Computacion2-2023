...
def thread_function(name):
    ...
    time.sleep(DELAY)
    print("Finalizando hilo %s, se acabó el tiempo (%s)" %
          (name, threading.current_thread().name))


if __name__ == "__main__":

    print("Iniciando programa principal (%s), antes de crear el thread" %
    ...
    print("\n ==> Tiene %d segundos para abrir otra terminal y ejecutar \" ps -eLf | grep %s | grep -v grep \" " %
          (DELAY, sys.argv[0]))
